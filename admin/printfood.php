...
$this->Cell(0,10,'รายการร้านอาหาร',0,0,'L');
...
$sql = "SELECT * FROM tbl_food f LEFT JOIN tbl_poi p ON (f.fd_poiid = p.poi_id)";
...
$pdf->Cell(10,50,$row['fd_id'],1,0,'C');
...
$y2=$pdf->GetY();
$pdf->SetXY($x,$y);
$pdf->Cell(110,10,$row['fd_name'],0,1,'L');
$pdf->SetFont('Angsana','',16);
$pdf->SetX($x);
$pdf->Cell(110,8,'ร้าน: '.$row['poi_name'],0,1,'L');
$pdf->SetX($x);
$pdf->MultiCell(110,8,'ราคา: '.number_format($row['fd_price']).' บาท',0,'L',false);
$id = $row['fd_poiid'];
$fd_id = $row['fd_id'];
$pic = '../assets/img/food/'.$id.'/'.$fd_id.'.png';
if (file_exists($pic)) {
