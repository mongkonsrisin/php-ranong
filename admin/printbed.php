...
$this->Cell(0,10,'รายการห้องพัก',0,0,'L');
...
$sql = "SELECT * FROM tbl_room rm LEFT JOIN tbl_poi poi ON (rm.rm_poiid = poi.poi_id)";
...
$pdf->Cell(10,50,$row['rm_id'],1,0,'C');
...
$pdf->SetFont('Angsana','',16);
$pdf->SetX($x);
$pdf->Cell(110,8,'จำนวนผู้เข้าพัก: '.$row['rm_maxpeople'].' คน',0,1,'L');
$pdf->SetX($x);
$pdf->MultiCell(110,8,'ราคา: '.number_format($row['rm_price']).' บาท',0,'L',false);
$pdf->SetX($x);
$pdf->MultiCell(110,8,'ขนาดห้อง: '.$row['rm_size'],0,'L',false);
$rm_poiid = $row['rm_poiid'];
$pic = '../assets/img/poi/s/'.$rm_poiid.'.png';
if (file_exists($pic)) {
