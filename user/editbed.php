...
//Authorize
if($row['rm_poiid'] != $_SESSION['id']) {
  echo '<meta http-equiv="refresh" content="0;url=allbed.php">';
  exit();
}
?>
<div class="container">
  ...
  if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(empty($rm_maxpeople) || empty($rm_price) || empty($rm_size)) {
      //Empty fields.
      ...
      //okay.
      $sql = "UPDATE tbl_room SET rm_maxpeople=?,rm_price=?,rm_size=? WHERE rm_id=?";
      $stmt = $con->prepare($sql);
      $stmt->bind_param("iisi",$rm_maxpeople,$rm_price,$rm_size,$id);
      $stmt->execute();
