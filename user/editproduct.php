  ...
  if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(empty($pro_name) || empty($pro_description) || empty($pro_price) || empty($pro_poiid) || empty($_FILES['pro_file'])) {
      ...
      allowOutsideClick: false
          ...
          $sql2 = "UPDATE tbl_product SET pro_name=? , pro_description=? , pro_price=? WHERE pro_id=?";
          $stmt2 = $con->prepare($sql2);
          $stmt2->bind_param("ssii",$pro_name,$pro_description,$pro_price,$id);
          $stmt2->execute();
