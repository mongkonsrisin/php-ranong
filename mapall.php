...
$(this).css({opacity: '1'});
      ...
      function showContent(id,cat,type){
        var filename = "";
        ...
        switch(cat) {
          case "1": filename = "hotel.php";
          break;
          case "2": 
          if(type == "1") {
            filename = "ecosystem.php";

          } else if (type == "2") {
            filename = "healthy.php";

          }
          break;
          case "3": filename = "food.php";
          break;
          case "4": filename = "product_description.php";
          break;
          default: break;
        }
        window.location = filename +'?id='+id;
      }
        ...
        for(var i = 0 ;i<json.length;i++){
		  ...
		  if(json[i].poi_skip_map!=1)
		 {
          ...
          if (latitude!="" && longtitude != "") {
            ...
            var img = "<a href='#' onClick=showContent('"+poi_id+"','"+json[i].poi_cat+"','"+json[i].poi_type+"');><img width='200'  height='200' src="+poi_photo+" style='border-radius:50%; padding:10px;'></a>";
            var marker_icon = {url:'assets/img/pin/pin'+poi_pin+'.png',
            scaledSize: new google.maps.Size(68, 60), // scaled size
            origin: new google.maps.Point(0,0), // origin
            anchor: new google.maps.Point(34, 60) // anchor
          };
          var markeroption = {map:map, html:img+'<br>'+"<a href='#' onClick=showContent('"+poi_id+"','"+json[i].poi_cat+"','"+json[i].poi_type+"');><div align='center' style='width:200px;margin:0 auto;text-align:center;padding:10px;'><div style='display:block; background-color:rgba(226, 226, 226,0.4);padding: 10px; border:1px solid #c6c4c4;' ><b>"+poi_name+"</b></div></div></a>", position:latlng, title: poi_name, animation: google.maps.Animation.DROP, icon:marker_icon };
